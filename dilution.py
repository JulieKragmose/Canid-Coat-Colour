...
def dilution(sample, ID, bamfile, outfile, translationDict):
	...
	for AApos, BPpos in allelePositions.items():
		...
		try:
			tview = 'samtools tview -d T -p chr25:'+BPpos+' -s STR /groups/hologenomics/shyam/data/forJulie/bams/'+sample+'.realigned.bam > MLPH.'+ID+'.pos'+AApos+'.txt'
		...
		viewfile = open('MLPH.'+ID+'.pos'+AApos+'.txt', 'r')
		codons = []
		lineNo = 0
